"""Request handler wrapper for A2A SDK DefaultRequestHandler with timing instrumentation."""
...
from a2a.server.events import (
    Event,
    EventConsumer as A2AEventConsumer,
    EventQueue,
...
class EventConsumer(A2AEventConsumer):
    """EventConsumer with reduced timeout for lower latency streaming.
    
    This is a minimal subclass that only overrides the timeout value.
    All other functionality comes from the A2A SDK's EventConsumer.
    """

    def __init__(self, queue: EventQueue):
        """Initialize with reduced timeout for faster event consumption."""
        super().__init__(queue)
        # Override timeout: reduced from 0.5s to 0.01s to minimize delay
        # The timeout is still needed to allow exception checking, but a smaller
        # value reduces latency for streaming responses
        self._timeout = 0.01


@trace_class(kind=SpanKind.SERVER)
class DefaultRequestHandler(RequestHandler):
    """Default request handler for all incoming requests with timing instrumentation.
    ...
    async def on_message_send_stream(
        ...
        """Default handler for 'message/stream' (streaming) with timing instrumentation.

        Starts the agent execution and yields events as they are produced
        by the agent.
        """
        stream_start = time.time()
        logger.info(f"[TIMING] A2A server: on_message_send_stream started")
        ...
        logger.info(f"[TIMING] A2A server: setup_message_execution completed: {setup_time:.3f}s, task_id={task_id}")
        ...
        try:
            async for event in result_aggregator.consume_and_emit(consumer):
                ...
                if not first_event_yielded:
                    first_event_time = time.time() - consume_start
                    time_from_setup = time.time() - stream_start
                    logger.info(f"[TIMING] A2A server: Time from consume_and_emit start to first event: {first_event_time:.3f}s")
                    logger.info(f"[TIMING] A2A server: Time from stream start to first event yield: {time_from_setup:.3f}s")
                    
                    # Log event type and state
                    if isinstance(event, Task):
                        event_type = "Task"
                        event_state = event.status.state.value if event.status else None
                    elif hasattr(event, 'status'):
                        event_type = type(event).__name__
                        event_state = event.status.state.value if hasattr(event.status, 'state') else None
                    else:
                        event_type = type(event).__name__
                        event_state = None
                    
                    logger.info(f"[TIMING] A2A server: First event type={event_type}, state={event_state}")
                ...
                if yield_time > 0.001:  # Only log if yield takes significant time
                    logger.info(f"[TIMING] A2A server: Event #{event_count} yield time: {yield_time:.3f}s")
            ...
            logger.info(f"[TIMING] A2A server: on_message_send_stream completed: total_time={total_time:.3f}s, events_yielded={event_count}")
