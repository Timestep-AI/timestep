	...
	async execute(
		...
		try {
			await withTrace(trace, async trace => {
				...
				if (stream.newItems?.length) {
					// https://openai.github.io/openai-agents-js/guides/results/#new-items
					// New items
					...
					for (const item of stream.newItems) {
						...
						if (
							...
							console.log(
								'Publishing event for RunHandoffCallItem or RunHandoffOutputItem',
							);
							const newRunItemEvent: TaskStatusUpdateEvent = {
								kind: 'status-update',
								taskId: taskId,
								contextId: contextId,
								status: {
				...
				const completedTask = await this.contextService.getTask(
					context.contextId,
					taskId,
				); // TODO: is completedTask missing full history?
				if (completedTask) {
