...
> **Note:** This app was copied and modified from the customer-support example app in the [openai-chatkit-advanced-samples](https://github.com/openai/openai-chatkit-advanced-samples) repository.
...
The backend for this demo lives in `apps/customer-support/backend` and ships with its own `pyproject.toml`.

```bash
cd apps/customer-support/backend
uv sync
export OPENAI_API_KEY="sk-proj-..."
uv run uvicorn app.main:app --reload --port 8000
```

The API exposes ChatKit at `http://127.0.0.1:8000/support/chatkit` and helper endpoints under `/support/*`.

### 2. Run the React frontend

```bash
cd apps/customer-support/frontend
npm install
npm run dev
```
...
From the `apps/customer-support` directory you can also run `npm start` to launch the backend (`uv sync` + Uvicorn) and frontend together. Ensure `uv` is installed and required environment variables (for example `OPENAI_API_KEY` and `VITE_SUPPORT_CHATKIT_API_DOMAIN_KEY`) are exported before using this shortcut.
...
2. Register that domain on the [domain allowlist page](https://platform.openai.com/settings/organization/security/domain-allowlist) and mirror it in `apps/customer-support/frontend/vite.config.ts` under `server.allowedHosts`.
3. Set `VITE_SUPPORT_CHATKIT_API_DOMAIN_KEY` to the key returned by the allowlist page and verify it surfaces in `apps/customer-support/frontend/src/lib/config.ts`.
